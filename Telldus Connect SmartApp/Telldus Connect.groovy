...
def success() {
	...
	def message = """
	<p>We have located your Telldus account.</p>
...
def initialize() {
	...
	settings.devices.each {
		...
		try {
			switch(detail?.type) {
				case 'sensor':
					log.debug "type: sensor"
					createChildDevice("Telldus Sensor", deviceId, detail.id, detail.name)
                    // (deviceFile, dni, name, label)
					break
                /*
				case 'NAModule1':
...
def getDeviceList() {
	...
	apiGet("api/sensor") { response -> 
        log.debug "response callback thingy"
        response.data.sensor.each { value ->
            def key = value.id
            log.debug "key: ${key}"
            deviceList[key] = "${value.name}"
            state.deviceDetail[key] = value
            state.deviceState[key] = value.name
        }
...
def createChildDevice(deviceFile, dni, name, label) {
	...
	try {
		...
		if(!existingDevice) {
			...
			def childDevice = addChildDevice("dianoga", deviceFile, dni, null, [name: name, label: label, completedSetup: true]) // TODO??
		} else {
